...
class StitchService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (id != null) {
            val relaxed = intent.getBooleanExtra(EXTRA_RELAXED, false)
    ...
    private fun process(sessionId: String, relaxed: Boolean) {
        val store = App.store
        ...
        try {
            ...
            }.stitch(store.sessionDir(sessionId), meta, store.equirectFile(sphereId), store.thumbFile(sphereId), relaxed)

            val sphere = Sphere(
                sphereId, name, System.currentTimeMillis(), result.width, result.height,
                result.defaultYaw, 0.0, mutableListOf(), false, result.totalShots, result.usedShots
            )
            store.add(sphere)
            ...
            Log.i(TAG, "Sphère $sphereId assemblée (${result.usedShots}/${result.totalShots} photos)")
    ...
    private fun notifyDone(sphere: Sphere, result: StitchResult) {
        val text = if (result.usedShots < result.totalShots)
            getString(R.string.stitch_notif_done_body, sphere.name) + " · " + getString(R.string.stitch_warning_dropped, result.totalShots - result.usedShots)
        else getString(R.string.stitch_notif_done_body, sphere.name)
